...
void base() {
    ...
    for (int i = 0; i < np; i++){
        for (int j = 0; j < np; j++) {
            ...
            for (int k = 0; k < n; k++) {
                if (fardetected_null[k] != 0) {
                    chisq[i][j] += fardetected_null[k] * TMath::Power(1 - nutonu(ICARUS_dist/energy[k]), 2);
                }
            }
    ...
    logchisqplot->SetTitle("#chi^{2} - Perfect Knowledge of Flux; log_{10}(sin^{2}(2#theta)); log_{10}(#Delta m^{2}); #chi^{2}");
    gStyle->SetPalette(1);
    logchisqplot->Draw("surf1");
    chisqcanvas->SaveAs("../Out/Base/chisqplot_base.root");
    ...
    gr_90->SetTitle("ICARUS Sensitivity - Perfect Knowledge of Flux; sin^{2}(2#theta); #Delta m^{2} (eV^{2})");
    ...
    canvas->SaveAs("../Out/Base/Sensitivity_Base.png");
    canvas->SaveAs("../Out/Base/Sensitivity_Base.root");

...
void wSBN() {
    ...
    vector <double> flux0(n), xsec0(n), nearflux_null(n), neardetected_null(n), farflux_null(n), fardetected_null(n);
    for (int i = 0; i < n; i++) {
        xsec0[i] = xsec_forflux[i] * 1e-42; //* nAr_atoms;
    ...
    cout << "Getting large batch of chi squareds." << endl;
    ...
    logchisqplot->SetTitle("#chi^{2} - Using SBN; log_{10}(sin^{2}(2#theta)); log_{10}(#Delta m^{2}); #chi^{2}");
    gStyle->SetPalette(1);
    logchisqplot->Draw("surf1");
    chisqcanvas->SaveAs("../Out/wSBN/chisqplot_SBN_2.root");
    ...
    vector <vector <double> > contour, sin_contour(target_dchisq.size()), dm2_contour(target_dchisq.size()), vecof2vecs(np, twozeros);
    ...
    gr_90->SetTitle("ICARUS Sensitivity - Using SBN; sin^{2}(2#theta); #Delta m^{2} (eV^{2})");
    ...
    canvas->SaveAs("../Out/wSBN/Sensitivity_SBN_2.root");
    canvas->SaveAs("../Out/wSBN/Sensitivity_SBN_2.png");
